...
/**静态标签项*/
public class StaticCssFileTag extends StaticBaseTag {
	/**
	 * 
	 */
	...
	public int doEndTag() throws JspException{
		if (this.getSetting().getAutoConcat() == null || !this.getSetting().getAutoConcat()){
			...
			try {
				writer.write(String.format("<link rel=\"stylesheet\" type=\"text/css\" href=\"%s%s\"/>", this.getSetting().getStyleHomeSite(), this.getValue()));
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			list.add(new StaticItem(StaticType.CssFile, value));
